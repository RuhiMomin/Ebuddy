...
async function viewAll(req, res) {
    let category = await categoryModel.viewAll(req.query, req.userData.permiss).catch((error) => { return { error } })
    console.log("cat", category)
    if (!category || (category && category.error)) {
        return res.render('category/view', { error: category.error })
    }
    return res.render('category/view', { category: category.data, page: category.page, limit: category.limit, total: category.total, permiss: req.userData.permiss })
}
...
async function update(req, res) {
    let category = await categoryModel.update(req.params.id, req.body, req.userData.id).catch((error) => { return { error } })
    if (!category || (category && category.error)) {
        let url = (category && category.data && category.data.id) ? '/category/' + category.data.id : '/category'
        return res.redirect(url)
    }
